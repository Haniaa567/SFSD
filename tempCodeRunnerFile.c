...
void sauvegarderFichierEnBinaire( fichier* maListe) {
    // Ouverture du fichier en mode écriture binaire
    FILE* fichier = maListe->fich;
...
void Ouvrir(fichier* f, char* nom_physique, char mode)
{
    if((mode == 'N') || (mode == 'n')) //Créer un nouveau fichier
    {
        ...
        f->supp_logique=true;
        f->taille_block=10;
        sauvegarderFichierEnBinaire(*nom_physique,*f);
    }
    else
    {
        if((mode == 'a') || (mode == 'A')) //Ouvrir un ancien fichier
        {
            ...
            chargerListeDepuisFichierBinaire(*nom_physique,fichier);
        }
    }
}
