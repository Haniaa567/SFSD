...
struct Donnee{
    char numero[10]; //Le numéro on l'utilise comme cle il est unique (comme matricule ou id....)
    char* data; // Un champ de taille variable que on peut l'initialiser comme on veut(comme nom ,wilya....)
...
void insert(Fichier* fichier,char* nom_physique,int numero,char* s)
 {
    ...
    //pour eviter l'ecrasement des eregistrement
...
void on_delete_button_clicked(GtkWidget *widget, gpointer data) {
    ...
    if (result == GTK_RESPONSE_OK) {
        const char *tmp = gtk_entry_get_text(GTK_ENTRY(entry));
        int cle = atoi(tmp);
        SuppressionLogiqueLOVC(&f,nom,cle);
        Ouvrir(&f, nom, 'A');
        on_refresh_button_clicked(NULL, NULL);
